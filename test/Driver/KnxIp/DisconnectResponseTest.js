...
describe("DisconnectResponse", function() {

    beforeEach(function() {
        this.channelId = 73;
        this.status = 0;
        this.response = new KnxIp.DisconnectResponse(this.channelId, this.status, this.endpoint);
    });
    ...
    it('parse sets correct values', function() {
        var buffer = new Buffer([0x06, 0x10, 0x02, 0x0a, 0x00, 0x08, 0x49, 0x00]),
    ...
    it('generates correct buffer', function() {
        var buffer = this.response.toBuffer(),
            expected = [0x06, 0x10, 0x02, 0x0a, 0x00, 0x08, 0x49, 0x00];
